...
## 7. Operativna pravila, query-ji i prosirene CSV tabele

Ovaj deo konkretizuje implementaciju da ne bude samo jedan veliki `if-else` blok.
Kljuc je da tabele imaju vise kolona koje nose pragove, prioritete, vremenske prozore i akcije.

### 7.1 Forward chaining pravila (koncept)

- Pravilo `DetectTachypneaByAgeBand`
  - iz tabele `AgeRespiratoryThreshold` bira opseg po uzrastu
  - ako je `respiratoryRate >= tachypneaThreshold`, ubacuje `TachypneaDetected` sa `riskWeight` i `defaultAction`

- Pravilo `DetectHypoxemiaConcern`
  - koristi `spo2ConcernThreshold` i `urgentSpo2Threshold` iz tabela
  - generise `OxygenationConcern` sa nivoom ozbiljnosti

- Pravilo `EscalateByCombinedSignals`
  - kombinuje `TachypneaDetected`, `WorkOfBreathingIncreased`, `HydrationConcern`, `RiskFactor`
  - racuna ukupan rizik i postavlja preporuku (`NEEDS_SAME_DAY_CLINICAL_REVIEW`, `URGENT_HOSPITAL_ASSESSMENT`, `EMERGENCY_RESPONSE`)

### 7.2 Backward chaining query-ji (koncept)

- `isSafeForHomeMonitoring(childId, now)`
  - vraca `true` samo ako nema aktivnih critical/high signala i ukupni rizik je ispod praga

- `getEscalationReasons(childId, now)`
  - vraca listu aktivnih razloga i `explanationCode` vrednosti koje su okinule eskalaciju

- `getRequiredAction(childId, now)`
  - vraca finalnu akciju sa najvecim prioritetom

### 7.3 CSV tabela: AgeRespiratoryThreshold (prosirena)

```csv
ruleId,ageBand,minAgeMonths,maxAgeMonths,tachypneaThreshold,tachycardiaThreshold,spo2ConcernThreshold,severeRrThreshold,reassessmentMinutes,riskWeight,defaultAction
RR_01,AGE_2_11_MONTHS,2,11,50,160,95,70,120,3,NEEDS_SAME_DAY_CLINICAL_REVIEW
RR_02,AGE_12_24_MONTHS,12,24,40,150,95,65,180,2,NEEDS_SAME_DAY_CLINICAL_REVIEW
RR_03,AGE_25_60_MONTHS,25,60,40,140,95,60,240,2,NEEDS_SAME_DAY_CLINICAL_REVIEW
```

### 7.4 CSV tabela: EscalationRiskModifier (prosirena)

```csv
riskFactor,severity,ageMinMonths,ageMaxMonths,escalationDelta,urgentSpo2Threshold,maxHomeMonitoringHours,requiresSeniorReview,followupMinutes,note
YOUNG_INFANT_UNDER_3_MONTHS,CRITICAL,2,3,2,94,6,true,60,Lower threshold for referral and observation
PREMATURITY,HIGH,2,60,1,94,8,true,90,Clinical deterioration may develop faster
CHRONIC_LUNG_DISEASE,HIGH,2,60,1,94,8,true,90,Higher risk of severe bronchiolitis or pneumonia course
HEMODYNAMIC_CONGENITAL_HEART_DISEASE,HIGH,2,60,1,94,8,true,90,Higher risk group in bronchiolitis guidance
IMMUNODEFICIENCY,HIGH,2,60,1,94,8,true,90,Lower tolerance for home monitoring
NEUROMUSCULAR_DISORDER,HIGH,2,60,1,94,8,true,90,Risk of secretion clearance problems
```

### 7.5 CSV tabela: RedFlagSeverity (prosirena)

```csv
flagType,severity,context,salienceLevel,riskPoints,minOccurrencesInWindow,windowMinutes,recommendedAction,mustNotifyImmediately,explanationCode
APNEA,CRITICAL,ANY,10000,10,1,5,EMERGENCY_RESPONSE,true,RED_FLAG_APNEA
CENTRAL_CYANOSIS,CRITICAL,ANY,10000,10,1,5,EMERGENCY_RESPONSE,true,RED_FLAG_CYANOSIS
CONVULSION,CRITICAL,ANY,10000,10,1,5,EMERGENCY_RESPONSE,true,RED_FLAG_CONVULSION
UNABLE_TO_DRINK,CRITICAL,RESPIRATORY_ILLNESS,9500,8,1,60,URGENT_HOSPITAL_ASSESSMENT,true,RED_FLAG_INTAKE
LETHARGY_OR_REDUCED_CONSCIOUSNESS,CRITICAL,ANY,9500,8,1,30,URGENT_HOSPITAL_ASSESSMENT,true,RED_FLAG_CONSCIOUSNESS
SEVERE_CHEST_INDRAWING,HIGH,RESPIRATORY_ILLNESS,9000,7,1,30,URGENT_HOSPITAL_ASSESSMENT,true,RESP_DISTRESS_INDRAWING
GRUNTING,HIGH,RESPIRATORY_ILLNESS,9000,7,1,30,URGENT_HOSPITAL_ASSESSMENT,true,RESP_DISTRESS_GRUNTING
PERSISTENT_LOW_SATURATION,HIGH,RESPIRATORY_ILLNESS,9000,7,2,60,URGENT_HOSPITAL_ASSESSMENT,true,RESP_HYPOXEMIA_PERSISTENT
REDUCED_INTAKE_WITH_RESPIRATORY_DISTRESS,MEDIUM,RESPIRATORY_ILLNESS,7000,4,1,180,NEEDS_SAME_DAY_CLINICAL_REVIEW,false,RESP_HYDRATION_CONCERN
```

### 7.6 Zasto ovako (umesto prostog if-else)

- Pravila ostaju ista, a medicinski pragovi se menjaju kroz CSV bez menjanja DRL koda.
- Prioriteti (`salienceLevel`) i vremenski obrasci (`windowMinutes`, `minOccurrencesInWindow`) se centralno konfigurisu.
- Isti parametri sluze i FC i CEP delu sistema.
- Odluka je objasnjiva jer svaki signal ima `riskPoints` i `explanationCode`.

## 8) homework iii implementation draft (aligned with exercise structure)

this is the concrete scope for homework iii and it stays inside fc bc cep templates only

### project structure

we follow the same split as in exercises

- model module
  - domain classes only
- kjar module
  - drl rules and templates
- service module
  - inserts facts runs rules prints results

### class diagram and data model

simple class map we can defend clearly

- childprofile
  - childid ageinmonths riskfactors
- illnessepisode
  - episodeid childprofile starttime status
- respiratoryassessmentevent
  - time respiratoryrate spo2 temperature chestindrawing nasalflaring grunting apnea cyanosis
- hydrationintakeevent
  - time intakepercent wetdiapercount poorfeeding
- riskassessment
  - level score reasons
- recommendation
  - action urgency explanation

### fc rules to implement now

we keep 4 rules so it is enough for defense and still realistic

1) detect_tachypnea_by_age_band
- uses template row by age band
- creates fact tachypneadetected

2) detect_oxygenation_concern
- checks spo2 threshold by age/context
- creates fact oxygenationconcern

3) detect_respiratory_distress
- chest indrawing or grunting or severe nasal flaring
- creates fact workofbreathingincreased

4) escalate_to_clinical_action
- combines previous derived facts and hydration concern
- creates recommendation with level same day urgent or emergency

### bc queries to implement now

we add 3 queries so explanation is easy during defense

- issafeforhomemonitoring(childid now)
- getescalationreasons(childid now)
- getrequiredaction(childid now)

### cep pattern to prepare for homework iv

for homework iii we only prepare event types and timeline fields
for homework iv we will activate one complex pattern like this

- persistent_tachypnea_in_12h
  - at least 2 respiratory events in 12h window above age threshold

### templates and csv tables

the point is to avoid giant if else blocks and keep parameters editable

#### table 1 age_respiratory_threshold.csv

```csv
ruleid,ageband,minagemonths,maxagemonths,tachypneathreshold,spo2concernthreshold,severerrthreshold,reassessmentminutes,riskweight,defaultaction
rr_01,age_2_11_months,2,11,50,95,70,120,3,needs_same_day_clinical_review
rr_02,age_12_24_months,12,24,40,95,65,180,2,needs_same_day_clinical_review
rr_03,age_25_60_months,25,60,40,95,60,240,2,needs_same_day_clinical_review
```

#### table 2 risk_modifier.csv

```csv
riskfactor,severity,ageminmonths,agemaxmonths,escalationdelta,urgentspo2threshold,maxhomemonitoringhours,requiresseniorreview,followupminutes,note
young_infant_under_3_months,critical,2,3,2,94,6,true,60,lower threshold for referral
prematurity,high,2,60,1,94,8,true,90,faster deterioration risk
chronic_lung_disease,high,2,60,1,94,8,true,90,higher severe course risk
hemodynamic_congenital_heart_disease,high,2,60,1,94,8,true,90,higher bronchiolitis risk
immunodeficiency,high,2,60,1,94,8,true,90,lower tolerance for home care
neuromuscular_disorder,high,2,60,1,94,8,true,90,secretion clearance risk
```

#### table 3 red_flag_severity.csv
...
### activation and output we show at defense

we prepare one runnable scenario in service module

- insert childprofile
- insert illnessepisode
- insert 2 respiratoryassessmentevent facts
- insert 1 hydrationintakeevent fact
- fireallrules
- print
  - derived facts tachypneadetected oxygenationconcern workofbreathingincreased
  - final recommendation
  - query results from 3 bc queries

### human explanation style in app output

we print explanations in short simple lines like this

- child is 10 months old so rr threshold is 50
- measured rr is 58 so tachypnea is detected
- spo2 is 94 so oxygenation concern is active
- chest indrawing is present so respiratory distress is active
- combined signals require urgent hospital assessment

this style is easy to defend because every line maps to one rule or one template row
